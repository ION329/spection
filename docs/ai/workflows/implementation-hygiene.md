# Workflow: Implementation Hygiene
...
Una sesión de IA que ha procesado product docs, arquitecturas y múltiples features
...
y uno que alucina patrones de otras features o contradice los contratos del engineering spec.
...
Si el engineering spec no está validado, detente. No hay implementación sin spec validada.
Vuelve al workflow `create-feature.md` y completa el Gate 2.
...
1. **`docs/ai/rules/constitution.md`** — las reglas de calidad de código (sección III) y context hygiene (sección V)
...
| `docs/spec/architecture/architecture.md` | Ya está destilado en el engineering spec y los ADRs |
| `docs/spec/features/[feature]-spec.md` | Ya está destilado en el engineering spec |
...
```
Lee el tasks file. Identifica la primera tarea con status: pending que no tenga
dependencias sin resolver. Implementa esa tarea siguiendo los contratos del engineering spec.
No implementes más de una tarea por prompt.
```

Trabaja tarea por tarea. Actualiza el `status` de cada tarea a `done` en el tasks file
...
- Si descubres que el engineering spec tiene un error: detente, corrígelo con el prompt `docs/ai/prompts/review.md`, valida el cambio con el ingeniero, y reinicia la sesión de implementación
...
> la constitución, el engineering spec de la feature, y el tasks file de la feature.
